...
Pushing a tag `vX.Y.Z` runs [.github/workflows/release.yml](../.github/workflows/release.yml), which:

1. Builds `i9950-printer-app` / `i9950-tool` with `VERSION=X.Y.Z`
2. Produces `i9950-printer-app-X.Y.Z-macos.pkg`
3. Creates a GitHub Release whose body is generated by [scripts/generate-release-notes.sh](../scripts/generate-release-notes.sh)
...
If the changelog has no `## [X.Y.Z]` section, the workflow **fails**. Add the section before tagging.
...
Useful flags: `--dry-run`, `--yes`, `--skip-build` (tag/notes only; CI attaches the package), `--no-push`.
...
5. Confirm the GitHub Actions **Release macOS package** run succeeds and the Release page shows changelog + install notes.
...
If you already published with `./scripts/release.sh`, CI will update the same release and replace the `.pkg` with the Actions-built asset.

Manual workflow dispatch builds a package artifact but **does not** create a GitHub Release (releases are tag-driven so notes stay tied to a versioned changelog entry).
...
| [.github/workflows/release.yml](../.github/workflows/release.yml) | CI build, test, upload / update GitHub Release |
